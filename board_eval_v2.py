...
def build_model():
    ...
    action_encoder = ActionPrefixEmbedder(input_dim=10, emb_dim=192, use_latent_condition=True,
        latent_dim=192, transformer_depth=3, transformer_heads=3, transformer_dim_head=64)
    ...
    predictor = ARPredictor(depth=6, mlp_dim=2048, input_dim=192, hidden_dim=192, output_dim=192, heads=8, dim_head=128)
    ...
    class CostModelWrapper(nn.Module):
        ...
        def get_cost(self, info_dict, action_candidates):
            with torch.no_grad():
                ...
                cur_emb = cur_emb.unsqueeze(1).expand(B, S, -1).reshape(B*S, -1)
                ...
                if npu_rknn is not None:
                    ...
                    # CPU 模式：自回归
                    act_emb1 = self.action_encoder(actions[:, :2], return_last_only=True, latent=cur_emb.unsqueeze(1))
                    pred1 = self.predictor(cur_emb.unsqueeze(1), act_emb1)
                    pred1_proj = self.pred_proj(pred1.reshape(B*S, -1)).reshape(B*S, 1, -1)
                    
                    act_emb2 = self.action_encoder(actions[:, 2:], return_last_only=True, latent=pred1_proj)
                    pred2 = self.predictor(pred1_proj, act_emb2)
                    pred2_proj = self.pred_proj(pred2.reshape(B*S, -1)).reshape(B*S, 1, -1)
                
                cost = ((pred2_proj - goal_emb.unsqueeze(1))**2).sum(dim=-1).squeeze(-1)
...
def load_npu_model():
    ...
    rknn = RKNNLite()
    rknn.load_rknn('/root/Fast-LeWorldModel/predictor_multistep_b300_fp16.rknn')
...
def main():
    ...
    npu_rknn = None
    if args.mode == 'npu':
        print(f"[BoardEval] Loading NPU model...", flush=True)
        npu_rknn = load_npu_model()
        print(f"[BoardEval] NPU model loaded", flush=True)
    ...
    for ep_idx, tc in enumerate(test_cases):
        ...
        for step in range(MAX_STEPS):
            ...
            for a_step in range(ACTION_BLOCK):
                ...
                if env.eval_state(goal_state, env._get_obs()):
                    success = True
                    break
                    success = True
                    break
