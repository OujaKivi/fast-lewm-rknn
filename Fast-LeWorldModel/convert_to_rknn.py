...
def _get_input_channels(onnx_path):
    """读取 ONNX 每个输入的通道数（shape 第二个维度）。"""
    import onnx
...
def convert_onnx_to_rknn(onnx_path, rknn_path, dtype="fp16", calib_samples=20, verbose=True):
    ...
    channels = _get_input_channels(onnx_path)
    ...
    if dtype == "fp16":
        ...
        # INT8：用随机数据做校准（仅用于流程测试）
        # 真实部署时应用真实数据校准
        calib_dir = "/tmp/rknn_calib"
        os.makedirs(calib_dir, exist_ok=True)
        # 生成随机校准数据（npy 格式）
        for i in range(calib_samples):
            # predictor 输入: latent (300,1,192) + act_emb (300,1,192)
            # action_encoder 输入: actions (300,5,2) + latent (300,1,192)
            # 用通用随机数据
            data = np.random.randn(1, 300, 192).astype(np.float32)
            np.save(os.path.join(calib_dir, f"calib_{i}.npy"), data)
        dataset = os.path.join(calib_dir, "dataset.txt")
        with open(dataset, "w") as f:
            for i in range(calib_samples):
                f.write(f"{calib_dir}/calib_{i}.npy\n")
