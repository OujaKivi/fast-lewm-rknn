...
### 6. CPU vs NPU 端到端对比（修复后）
| 指标 | 板端 CPU | 板端 NPU | 差异 |
|------|---------|---------|------|
| 成功率 | 100% | 100% | 一致 ✓ |
| 规划时间 | 17.75s | 19.00s | NPU 慢 7% |
| 平均 reward | -318.3 | -318.3 | 完全一致 ✓ |

**关键结论**：
- NPU 精度无损：FP16 量化不影响规划效果
- NPU 当前实现反而更慢：两次 NPU 推理 + 数据搬运开销抵消了计算加速
- 原因：当前 NPU 用两次推理模拟自回归，每次都有 CPU↔NPU 数据搬运
...
### P0：优化 NPU 调用方式
1. 合并两次自回归预测为一次多步预测（直接用完整5个block的action_emb输入）
2. 融合 pred_proj 到 NPU 模型（减少CPU投影开销）
3. 预期：NPU 规划时间从 19.0s 降到 12-14s
